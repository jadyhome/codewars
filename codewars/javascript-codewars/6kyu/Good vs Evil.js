...
function goodVsEvil(good, evil) {
  let ggood = { 0: "1", 1: "2", 2: "3", 3: "3", 4: "4", 5: "10" };
  let eevil = { 0: "1", 1: "2", 2: "2", 3: "2", 4: "3", 5: "5", 6: "10" };

  good = good.split(" ");
  evil = evil.split(" ");
  let goodTeam = good
    .map((value, index) => value * ggood[index])
    .reduce((a, b) => a + b);
  let evilTeam = evil
    .map((value, index) => value * eevil[index])
    .reduce((c, d) => c + d);

  if (goodTeam > evilTeam) {
    return "Battle Result: Good triumphs over Evil";
  } else if (goodTeam < evilTeam) {
