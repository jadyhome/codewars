...
// is ** at most 2 times higher than the number of developers representing any of the remaining programming languages**; or
// false otherwise.
...
function isLanguageDiverse(list) {
  let langs = list.map((l) => l.language);

  let numOfLangs = {};

  for (const el of langs) {
    numOfLangs[el] = numOfLangs[el] ? numOfLangs[el] + 1 : 1;
  }

  let valuesCounts = Object.values(numOfLangs).sort((a, b) => b - a);
  let maxLang = valuesCounts[0];

  if (valuesCounts.every((value) => value * 2 >= maxLang)) {
    return true;
  } else {
    return false;
  }
