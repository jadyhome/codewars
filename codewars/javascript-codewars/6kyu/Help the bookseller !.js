...
function stockList(listOfArt, listOfCat) {
  ...
  let filteredListOfArt = listOfArt.filter((code) =>
    listOfCat.includes(code[0])
  );

  let categories = {};
  filteredListOfArt.forEach((book) => {
    categories[book[0]] = (categories[book[0]] || 0) + +book.split(" ")[1];
  });

  let stocklist = listOfCat
    .map((letter) => {
      return `(${letter} : ${categories[letter] || 0})`;
    })
    .join(" - ");
