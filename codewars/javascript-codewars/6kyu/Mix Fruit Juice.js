...
// Those extra special fruits cost $9 per each. There is no limit on how many fruits she/he picks.
// The price of a cup of juice is the mean of price of chosen fruits.
// In case of decimal number (ex. $5.99),
// output should be the nearest integer (use the standard rounding function of your language of choice).
...
function mixFruit(arr) {
  ...
  let regularPrice = 5;
  let specialPrice = 7;
  let extraSpecialPrice = 9;

  let prices = [];

  arr = arr.map((fruits) => fruits.toLowerCase());

  arr = arr.map((p) => {
    if (regular.includes(p)) {
      prices.push(regularPrice);
    } else if (special.includes(p)) {
      prices.push(specialPrice);
    } else {
      prices.push(extraSpecialPrice);
    }
  });
