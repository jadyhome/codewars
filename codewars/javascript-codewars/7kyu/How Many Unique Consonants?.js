...
function countConsonants(str) {
  str = str.split("").map((item) => item.toLowerCase());
  let unique = str.filter((letter, index) => str.indexOf(letter) === index);
  let consonants = unique.filter((x) => !x.match(/[aeiou0-9!\s+]/gi));
